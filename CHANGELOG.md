...
- **左侧列表拖动排序**：排序后按发票对象重新定位预览页和槽位，支持多月份发票、排版多份及正/倒序显示，序号与实际顺序保持一致
- **侧栏金额显示**：侧栏宽度改为响应式自适应，金额和排版份数徽章允许换行，避免内容被截断
- **NSIS 安装包图标**：安装器图标统一使用 `src-tauri/icons/icon.ico`
...
- **侧栏选择与筛选回归**：统一当前可见且可选文件的判定，修复全选、取消全选、单项勾选和列表计数在类型/格式/打印状态筛选下不一致的问题
- **列表点击联动回归**：移除合并后重复定义的列表点击处理，恢复拖拽后的点击保护与右键联动，不再意外改变勾选状态
- **重复添加文件**：修复添加发票入口重复加载同一批文件的问题
- **重复界面入口**：移除重复的文件输入框和全选按钮

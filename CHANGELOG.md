...
- **文件夹拖放导入**：支持拖入文件夹，自动递归查找发票文件并筛选 PDF、图片、OFD、XML 等支持格式
- **导入结果汇总**：底部 toast 显示添加文件数、成功解析发票数和跳过的不支持文件数，保持显示并支持手动关闭

### 🐛 修复

- **跳过非发票 TIF/TIFF**：OCR 识别后自动移除无发票特征的 TIF/TIFF 图片，避免普通缓存图片进入发票列表
...
- **左侧列表拖拽排序**：列表和缩略图方格视图均支持拖动发票排序，拖到目标项上半区插入前面、下半区插入后面；复选框和操作按钮不触发排序

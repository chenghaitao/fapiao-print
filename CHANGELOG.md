...
## v2.6.7 — 医疗收费票据专类 + 多页发票逻辑票聚合

_2026-09-21_

### ✨ 新功能

- **医疗收费票据专类**：类型检测链路新增 `medical` 档（`ticket > toll > medical > nontax > vat > ride > unknown`），强标记「医疗门诊/住院/急诊收费票据|医疗收费明细」直判，**必须先于 nontax 判定**（否则票面的「票据代码/票据号码/交款人」会先把它判成非税票据）；复用非税提取链路（金额合计 / 票据号码 / 交款人），`getMedicalLabel()` 细分门诊/住院/急诊标签；报销口径只取金额合计，不识别自费/统筹等支付分解字段；侧栏筛选新增「医疗」维度，与车票/通行费同样并入 `resolveInvoiceType` 单一真源
- **医疗明细页附件化**：第二页起的「医疗收费明细」长清单由 `isMedicalDetailPage()` 双确认（`医疗收费明细` + `所属电子票据号码`，「所属」前缀是明细页独有特征）识别，`finalizeMedicalDetailPages()` 从列表移除——只保留汇总首页，不参与排版/统计/去重；主票挂「附N页明细」徽章并 toast 说明（幂等 `_medDetailHandled`）

### 🐛 修复

- **多页发票逻辑票聚合**（issue #40）：跨页发票按发票号聚合为一张「逻辑票」，统计/去重/删除/类型判定全部按逻辑票整组处理——同一张跨页发票绝不拆开单删（防打印缺页）；列表新增续页徽章（非合计页「续 2/3」、合计页「共 3 页」）
- **重复发票计数按逻辑票而非物理页**：同一张跨页发票的多个页面只计一次，避免重复数虚高；`_dup` 标记仍按页整组置位，保证筛选与勾选删除口径一致
- **医疗明细页排除先于自动去重**：明细页与主票发票号相同，若先自动去重会被整页删掉，导致主票不挂「附N页明细」徽章且 toast 提示误导；现调整为明细页排除优先于去重

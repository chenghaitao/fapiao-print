...
## v2.6.9 — 同步上游 v2.6.8（单票工具条越界修复 + 一键删除）

_2026-09-24_

### 🔀 同步上游

- 合并上游 erma0 v2.6.8（issue #43）：**单票浮动工具条越界修复**（按预览区可视宽度左右钳制，各留 8px）+ **工具条一键删除**（工具条末尾新增「✕ 删除」，删完自动清空选中）

### 📌 说明

- **本地功能修改点全部保留**：类型筛选按钮按文字宽度自适应、更新弹窗按钮换行、本地编译脚本、NSIS 安装图标、前端 PNG 图标资源
- 冲突仅 `CHANGELOG.md` 与 `README.md`（两边 v2.6.8 条目不同），已合并双方内容；代码文件自动合并成功

---

## v2.6.8 — 单票工具条越界修复 + 一键删除 + 类型筛选按钮宽度自适应 + 更新弹窗按钮换行
...
- **类型筛选按钮「通行费」文字贴边**：类型行 8 个按钮此前按 `flex:1` 等分宽度，37px 的按钮要放下 33px 的 3 个字，左右各只剩 1px。现改为**按文字宽度铺底、余量再均分**（`flex-basis: auto`），长标签自动获得等量左右留白（通行费 37 → 47px）；类型 / 格式 / 状态三行均不再溢出或换行，状态行「未打印 / 已打印」同步受益（原先与「全部 / 重复」一律等宽）
- **更新提示弹窗按钮溢出**：次要按钮与主按钮此前挤在同一行，窄窗口下会被挤出弹窗；现改为三个次要按钮一行、主按钮独占一行

### 📌 说明

- 同步上游 erma0 v2.6.8（issue #43）；本地编译脚本、NSIS 安装图标与前端 PNG 图标资源保持不变
...
- **更新提示弹窗按钮溢出**：三个次要按钮一行、主按钮独占一行，避免窄窗口下按钮挤出弹窗
